      ...
      >
        {item.coverImage && (
          <div
            ...
            data-cursor="pointer"
            onClick={() => setLightboxSrc(item.coverImage)}
          >
            <img
              src={item.coverImage}
              alt=""
        ...
        <div className="flex items-center gap-3">
          {item.authorAvatar ? (
            <img src={item.authorAvatar} alt="" className="h-10 w-10 rounded-full object-cover" />
          ) : (
            <div
              className="flex h-10 w-10 items-center justify-center rounded-full font-jakarta text-[14px] font-bold text-white"
          ...
          <a
            ...
            data-cursor="pointer"
            className="flex-shrink-0 text-text-dim transition-colors hover:text-text-primary"
        ...
        {item.title && (
          <p className="mt-3 line-clamp-2 font-jakarta text-[17px] font-bold leading-snug text-text-primary">
            {item.title}
          </p>
        )}

        <p className="mt-2 line-clamp-3 font-inter text-[13px] leading-relaxed text-text-muted">
          {item.snippet}
        </p>
    ...
    return (
      <main className="relative z-10 mx-auto flex min-h-[50vh] w-full max-w-5xl flex-col items-center justify-center px-4 py-12 text-center">
        ...
        <motion.button
          ...
          data-cursor="pointer"
          onClick={() => void load()}
          className="mt-4 rounded-full px-5 py-2 font-inter text-[14px] font-semibold text-white"
          style={{ background: 'linear-gradient(135deg, #7c3aed, #06b6d4)' }}
    ...
    return (
      <main className="relative z-10 mx-auto flex min-h-[50vh] w-full max-w-5xl flex-col items-center justify-center px-4 py-12 text-center">
        ...
        <Link
          to="/collections"
          data-cursor="pointer"
          className="mt-5 inline-flex items-center rounded-full border px-4 py-2 font-mono text-[11px] uppercase tracking-[0.08em] text-text-muted transition-colors hover:text-text-primary"
  ...
  return (
    <main className="relative z-10 mx-auto w-full max-w-5xl px-4 py-12">
      ...
      <motion.div
        ...
        className="rounded-[28px] border p-6 backdrop-blur-xl md:p-8"
        style={{ background: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
      >
        ...
        {collection.description && (
          <p className="mt-3 max-w-2xl font-inter text-[15px] leading-relaxed text-text-muted">
            {collection.description}
          </p>
        )}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 font-mono text-[11px] text-text-dim">
          <span className="inline-flex items-center gap-1">
            <BookOpen className="h-3 w-3" /> {collection.itemCount} article{collection.itemCount !== 1 ? 's' : ''}
          </span>
          <span className="text-border-subtle">·</span>
          <span className="inline-flex items-center gap-1">
            <Eye className="h-3 w-3" /> {collection.viewCount} view{collection.viewCount !== 1 ? 's' : ''}
          </span>
          <span className="text-border-subtle">·</span>
          <span className="inline-flex items-center gap-1">
            <Clock3 className="h-3 w-3" /> Created {formatDate(collection.createdAt)}
          </span>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <motion.button
            ...
            data-cursor="pointer"
            onClick={() => {
              const link = `${window.location.origin}${window.location.pathname}#/collections/${collection.id}`
      ...
      <motion.div
        ...
        className="mt-10 flex items-center justify-center gap-2 py-6 font-mono text-[11px] text-text-dim"
      >
        <Lock className="h-3 w-3" />
        This collection is locked · Created with ArticleX
      </motion.div>
    </main>
  )
}
