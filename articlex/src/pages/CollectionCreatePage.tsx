...
export function CollectionCreatePage() {
  ...
  const handleAddArticle = useCallback(async () => {
    ...
    try {
      ...
      if (!tweetId) {
        setAddError('Please paste a valid X/Twitter URL.')
        return
      }

      if (items.some((i) => i.tweetId === tweetId)) {
        setAddError('Already added.')
        return
      }

      if (items.length >= 50) {
        setAddError('Maximum 50 articles per collection.')
        return
      }
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      if (!collection) {
        setImportMsg('Collection not found.')
        return
      }
      ...
      if (newItems.length === 0) {
        setImportMsg('All articles from that collection are already added.')
        return
      }
      ...
      setImportMsg('Failed to import collection.')
    } finally {
  ...
  }, [name, description, items, creating])
  ...
  const canCreate = name.trim().length > 0 && name.trim().length <= 60 && items.length > 0 && !creating
    ...
    <main className="relative z-10 mx-auto w-full max-w-3xl px-4 py-12">
      ...
      >
        ...
        <p className="mt-3 max-w-xl font-inter text-[15px] text-text-muted">
          Curate a locked reading list of X/Twitter articles. Once created, it cannot be edited.
        </p>
      </motion.div>
      ...
      <AnimatePresence>
        {createdId && (
          ...
          >
            ...
            <p className="mt-4 font-jakarta text-xl font-bold text-text-primary">Collection Created & Locked</p>
            <p className="mt-1 flex items-center justify-center gap-1.5 font-mono text-[11px] text-text-dim">
              <Lock className="h-3 w-3" /> Permanently read-only
            </p>
            ...
            <div className="mx-auto mt-5 flex max-w-md items-center gap-2 rounded-xl border p-2" style={{ borderColor: 'var(--border-subtle)', background: 'var(--bg-elevated)' }}>
              <input
                readOnly
                value={shareUrl}
                className="flex-1 bg-transparent px-2 font-mono text-[12px] text-text-muted outline-none"
              ...
              <motion.button
                ...
                data-cursor="pointer"
                onClick={handleCopy}
                className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-mono text-[11px] text-white"
                style={{ background: 'linear-gradient(135deg, #7c3aed, #06b6d4)' }}
            ...
            <motion.button
              type="button"
              data-cursor="pointer"
              onClick={() => navigate(`/collections/${createdId}`)}
      ...
      {!createdId && (
        <>
          ...
          <motion.div
            ...
            className="mt-8 rounded-[28px] border p-6 backdrop-blur-xl"
            style={{ background: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
          >
            ...
            <input
              ...
              placeholder="My favourite AI threads"
              maxLength={60}
              className="mt-2 w-full rounded-xl border bg-bg-elevated px-4 py-3 font-inter text-[14px] text-text-primary outline-none transition-colors placeholder:text-text-dim focus:border-accent-violet/60"
            ...
            <label className="mt-5 block font-mono text-[11px] uppercase tracking-[0.08em] text-text-muted">
              Description <span className="normal-case text-text-dim">(optional)</span>
            </label>
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="A collection of the best AI threads on X"
              maxLength={200}
              rows={2}
              className="mt-2 w-full resize-none rounded-xl border bg-bg-elevated px-4 py-3 font-inter text-[14px] text-text-primary outline-none transition-colors placeholder:text-text-dim focus:border-accent-violet/60"
              style={{ borderColor: 'var(--border-subtle)' }}
            />
          ...
          >
            <div className="flex items-center gap-3">
              ...
              {items.length > 0 && (
                <span
                  className="rounded-full px-2 py-0.5 font-mono text-[10px] text-text-muted"
            ...
            <div className="mt-4 flex gap-2">
              <input
                ...
                type="url"
                value={articleUrl}
                onChange={(e) => { setArticleUrl(e.target.value); setAddError(null) }}
                onKeyDown={(e) => { if (e.key === 'Enter') void handleAddArticle() }}
                placeholder="Paste an X/Twitter URL..."
                disabled={addLoading}
                className="flex-1 rounded-xl border bg-bg-elevated px-4 py-3 font-inter text-[14px] text-text-primary outline-none transition-colors placeholder:text-text-dim focus:border-accent-violet/60 disabled:opacity-60"
                style={{ borderColor: 'var(--border-subtle)' }}
              />
              <motion.button
                type="button"
                ...
                disabled={addLoading || !articleUrl.trim()}
                className="inline-flex items-center gap-1.5 rounded-xl px-5 py-3 font-inter text-[14px] font-semibold text-white disabled:opacity-50"
                style={{ background: 'linear-gradient(135deg, #7c3aed, #06b6d4)' }}
                whileHover={addLoading ? undefined : { scale: 1.03 }}
                whileTap={addLoading ? undefined : { scale: 0.97 }}
              >
            ...
            <AnimatePresence>
              {addError && (
                <motion.p
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="mt-2 flex items-center gap-1.5 font-inter text-[12px] text-red-400"
                >
                ...
                {items.map((item, index) => (
                  ...
                  >
                    {item.authorAvatar ? (
                      ...
                      <div
                        className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full font-jakarta text-[13px] font-bold text-white"
                        style={{ background: 'linear-gradient(135deg, #7c3aed, #06b6d4)' }}
                      >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate font-inter text-[13px] font-semibold text-text-primary">
                        {item.authorName}
                        <span className="ml-1.5 font-mono text-[11px] font-normal text-text-dim">@{item.authorHandle}</span>
                      </p>
                      <p className="truncate font-inter text-[12px] text-text-muted">
                    ...
                    {item.coverImage && (
                      <img
                        src={item.coverImage}
                        alt=""
                        className="hidden h-10 w-[60px] flex-shrink-0 rounded-lg object-cover sm:block"
                      />
                    )}
                    ...
                    <div className="flex flex-shrink-0 items-center gap-1">
                      <button
                        ...
                        disabled={index === 0}
                        className="rounded-md p-1 text-text-dim transition-colors hover:text-text-primary disabled:opacity-30"
                      >
                      ...
                      <button
                        type="button"
                        data-cursor="pointer"
                        onClick={() => handleRemoveItem(item.tweetId)}
                        className="rounded-md p-1 text-text-dim transition-colors hover:text-red-400"
                      >
            ...
            {items.length === 0 && (
              <p className="mt-4 text-center font-inter text-[13px] text-text-dim">
                ...
                >
                  <div className="mt-4 flex gap-2">
                    <input
                      ...
                      placeholder="Collection link or ID..."
                    ...
                    <motion.button
                      ...
                      className="rounded-xl px-5 py-3 font-inter text-[14px] font-semibold text-white disabled:opacity-50"
                      style={{ background: 'linear-gradient(135deg, #7c3aed, #06b6d4)' }}
                      whileHover={importLoading ? undefined : { scale: 1.03 }}
                      whileTap={importLoading ? undefined : { scale: 0.97 }}
                    >
                      {importLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Import'}
                    </motion.button>
                  </div>
                  {importMsg && (
                    <p className={`mt-2 font-inter text-[12px] ${importMsg.includes('Imported') ? 'text-emerald-400' : 'text-text-dim'}`}>
                      {importMsg}
                    </p>
                  )}
                </motion.div>
          ...
          >
            <AnimatePresence>
              {createError && (
                <motion.p
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="mb-3 flex items-center gap-1.5 font-inter text-[12px] text-red-400"
                >
            ...
            <motion.button
              ...
              disabled={!canCreate}
              className="flex w-full items-center justify-center gap-2 rounded-full px-6 py-3.5 font-inter text-[15px] font-semibold text-white disabled:opacity-40"
              style={{ background: 'linear-gradient(135deg, #7c3aed, #06b6d4)' }}
              whileHover={canCreate ? { scale: 1.02, boxShadow: '0 0 40px rgba(124,58,237,0.3)' } : undefined}
